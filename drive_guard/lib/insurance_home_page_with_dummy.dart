...
List<Map<String, dynamic>> _generateDummyTrips(int count, int userId) {
  final now = DateTime.now();
  final random = Random();
  final baseScore = random.nextDouble() * 30 + 70; // Base score between 70-100
  
  return List.generate(count, (index) {
    final distance = 5.0 + random.nextDouble() * 20.0;
    final duration = distance * (2 + random.nextDouble());
    final avgSpeed = 25.0 + random.nextDouble() * 30.0;
    final maxSpeed = avgSpeed + 5.0 + random.nextDouble() * 15.0;
    final variation = (random.nextDouble() - 0.5) * 20;
    final tripScore = (baseScore + variation).clamp(50.0, 100.0);
    final brakeScore = (tripScore * (0.8 + random.nextDouble() * 0.4)).clamp(0.0, 100.0);
    final accelScore = (tripScore * (0.8 + random.nextDouble() * 0.4)).clamp(0.0, 100.0);

    // Ensure no null values
    return {
      'trip_id': 'dummy_${index + 1000}',
      'user_id': userId,
      'start_time': now.subtract(Duration(days: index)).toIso8601String(),
      'distance': distance,
      'duration': duration,
      'avg_speed': avgSpeed,
      'max_speed': maxSpeed,
      ...
      'is_dummy': true,
    }..removeWhere((key, value) => value == null); // Remove any null values
  });
}

List<Map<String, dynamic>> _combineRealAndDummyTrips(
  List<Map<String, dynamic>> realTrips,
  int userId, {
  int minTrips = 15,
}) {
  // First validate that we have at least the minimum trips
  if (realTrips.length >= minTrips) {
    return realTrips;
  }
  
  final dummyCount = minTrips - realTrips.length;
  final dummyTrips = _generateDummyTrips(dummyCount, userId);
  
  return [...realTrips, ...dummyTrips];
}
...
void _selectUser(Map<String, dynamic> user) async {
  ...
  // Load trips first, then score (which will use the trips data)
  await _loadUserTrips(_searchedUserId);
  await _loadUserScore(_searchedUserId);
}
...
Widget _buildUserScoreCard({
  ...
  final score = _userScore?['score'] != null 
      ? (_userScore!['score']).round()
      : 0;
  final brakeScore = _userScore?['brake_score'] != null
      ? (_userScore!['brake_score'] ).round()
      : 0;
  final accelScore = _userScore?['accel_score'] != null
      ? (_userScore!['accel_score']).round()
      : 0;
            ...
            Column(
              children: [
                ...
                if (showFullDetails && forWebLayout) ...[
                  ...
                  SizedBox(height: 16),
                  if (_userScore!.containsKey('accel_score'))
                    _buildScoreDetailRow('Acceleration', accelScore),
                  if (_userScore!.containsKey('brake_score'))

    _buildScoreDetailRow('Braking', brakeScore),
                  if (_userScore!.containsKey('trip_score'))

    _buildScoreDetailRow('Overall Score', score),
                ],
...
Widget _buildCalculationDetails(BuildContext context, Map<String, dynamic> scoreData) {
  ...
  final allTrips = _userTrips; // Use the combined trips list instead of scoreData['all_trips']
  final hasTrips = allTrips.isNotEmpty;

  // Calculate averages from combined real and dummy trips
  final totalTrips = allTrips.length;
  final totalDistance = allTrips.fold(0.0, (sum, trip) => sum + (trip['distance'] ?? 0.0));
  final avgScore = allTrips.fold(0.0, (sum, trip) => sum + (trip['trip_score'] ?? 0.0)) / totalTrips;
  final avgBraking = allTrips.fold(0.0, (sum, trip) => sum + (trip['brake_score'] ?? 0.0)) / totalTrips;
  final avgAccel = allTrips.fold(0.0, (sum, trip) => sum + (trip['accel_score'] ?? 0.0)) / totalTrips;
  ...
  return Column(
    ...
    children: [
      ...
      if (hasTrips) ...[
        ...
        // Trip data summary - now includes dummy trips
        Text('Trip Data Summary ($totalTrips trips):',
            style: TextStyle(fontWeight: FontWeight.w500)),
        SizedBox(height: 8),
            ...
            Row(
              ...
              children: [
                _buildTripStat('Avg. Braking', 
                  '${avgBraking.toStringAsFixed(1)}'),
                _buildTripStat('Avg. Acceleration', 
                  '${avgAccel.toStringAsFixed(1)}'),
              ],
            ),
          ],
        ),
              ...
              child: _isLoadingTrips
                  ...
                  : _userTrips.isEmpty
                      ...
                      : ListView.separated(
                          ...
                          // Inside the ListView.builder or ListView.separated in _buildUserTripsCard
itemBuilder: (context, index) {
  final trip = _userTrips[index];
  final isDummy = trip['is_dummy'] == true;
  ...
  return Container(
    ...
    child: ListTile(
      ...
      dense: true,
      leading: Icon(
        isDummy ? Icons.help_outline : Icons.directions_car,
        size: isWeb ? 24 : 20,
        color: isDummy ? Colors.orange : Colors.blue.shade800,
      ),
      title: Row(
        children: [
          Text(startTime),
...
Future<void> _loadUserScore(String userId) async {
  ...
  try {
    ...
    // Function to properly convert scores
    double convertScore(dynamic score) {
      if (score == null) return 0.0;
      double value = score is int ? score.toDouble() : score;
      // If score appears to be multiplied by 100 (like 6123 instead of 61.23)
    ...
    // Calculate combined scores from all trips (real + dummy)
    final allTrips = _userTrips.isNotEmpty ? _userTrips : scoreData['all_trips'] ?? [];
    final totalTrips = allTrips.length;
    
    if (totalTrips > 0) {
      final totalScore = allTrips.fold(0.0, (sum, trip) => sum + convertScore(trip['trip_score']));
      final totalBrake = allTrips.fold(0.0, (sum, trip) => sum + convertScore(trip['brake_score']));
      final totalAccel = allTrips.fold(0.0, (sum, trip) => sum + convertScore(trip['accel_score']));
      
      final avgScore = totalScore / totalTrips;
      final avgBrake = totalBrake / totalTrips;
      final avgAccel = totalAccel / totalTrips;

      setState(() {
        _userScore = {
          ...scoreData,
          'score': avgScore,
          'brake_score': avgBrake,
          'accel_score': avgAccel,
          'trip_count': totalTrips,
          'all_trips': allTrips,
          'calculation': scoreData['calculation'] ?? {
            'formula': "Overall Score = Average of all trip scores",
            'weights': {
              'braking': 0.4,
              'acceleration': 0.4,
              'speed': 0.2,
      ...
      // Convert existing scores if no trips
      setState(() {
        _userScore = {
          ...scoreData,
          'score': convertScore(scoreData['score']),
          'brake_score': convertScore(scoreData['brake_score']),
          'accel_score': convertScore(scoreData['accel_score']),
        };
        _isLoadingScore = false;
      });
    }
...
Future<void> _loadUserTrips(String userId) async {
  ...
  try {
    ...
    setState(() {
      _userTrips = _combineRealAndDummyTrips(trips, userIdInt);
    ...
    setState(() {
      _isLoadingTrips = false;
      _searchError = 'Failed to load trips';
    });
  }
}
}