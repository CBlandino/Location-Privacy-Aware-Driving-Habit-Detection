      ...
      child: Column(
        ...
        children: [
            ...
            Padding(
              ...
              child: Text(
                'Note: This is sample trip data for demonstration purposes',
                style: TextStyle(
                  color: Colors.orange,
...
static Future<List<Map<String, dynamic>>> getUserTrips(String userId, {String? sortBy}) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // Debug print to see raw API response
      print('Raw trips data: $trips');
      
      return trips.map((trip) {
        return {
          'trip_id': trip['trip_id'] ?? 'unknown_${DateTime.now().millisecondsSinceEpoch}',
          'user_id': trip['user_id'] ?? userId,
          'start_time': trip['start_time'] ?? DateTime.now().toIso8601String(),
          'distance': (trip['distance'] ?? trip['trip_distance'] ?? 0).toDouble(),
          'duration': (trip['duration'] ?? trip['trip_duration'] ?? 0).toDouble(),
          'avg_speed': (trip['avg_speed'] ?? trip['average_speed'] ?? 0).toDouble(),
          'max_speed': (trip['max_speed'] ?? trip['maximum_speed'] ?? 0).toDouble(),
          'brake_score': (trip['brake_score'] ?? trip['braking_score'] ?? 0).toDouble(),
          'accel_score': (trip['accel_score'] ?? trip['acceleration_score'] ?? 0).toDouble(),
          'trip_score': (trip['trip_score'] ?? trip['overall_score'] ?? 0).toDouble(),
          'is_dummy': false,
        };
      }).toList();
    } else {
